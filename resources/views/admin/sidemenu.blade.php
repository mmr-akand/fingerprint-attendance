...
<ul class="aside-nav">
    ...
    <li class="aside-nav-item nav-has-dropdown {{ Request::is('admin/panel/union*') ? 'active' : '' }}">
        <a href="javascript:void(0)"><i class="icofont icofont-gift"></i> <span class="nav-text">Unions</span> </a>
        <ul class="aside-nav-sub">
            @foreach($unions as $union)
                <li class="aside-nav-item {{ Request::is('/admin/panel/union') ? 'active' : '' }} "><a href="/admin/panel/union/"> <span class="nav-text">{{$union->name}}</span> </a></li>
            @endforeach
        </ul>
    </li> 

    <li class="aside-nav-item {{ Request::is('admin/panel/school*') ? 'active' : '' }}"><a href="/admin/panel/school/index"><i class="icofont icofont-university"></i> <span class="nav-text">All Schools</span> </a></li>
